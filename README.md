...
- **Plan** — Gabriella, Anugra, Amil or Sohail split the team into groups for a day (by default Team A doing interviews on the Google Forms, Team B doing metrics in this app), pick members, a lead, sites and instructions, and publish. Everyone sees it live, with their own group highlighted and a "Your group today" card on Today. The survey group gets direct links to the three Google Forms. Plans are stored as `catId: "plan"` docs in the existing `entries` collection, so no Firestore rule change was needed; the app keeps them out of the Log, Report and Dashboard. The publish restriction is only in the app, like the name picker, not a real permission.
